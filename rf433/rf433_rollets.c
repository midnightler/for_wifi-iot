...
uint16_t count_cycle;// кол-во циклов дл€ роллеты (врем€ откр/закр)
uint16_t count1_cycle  = 100;// полное откр/закр
uint8 count2_cycle = 0; // кол-во циклов дл€ групп
uint16_t tik1_cycle = 0; //циклы дл€ роллеты
uint8 tik2_cycle = 0; // циклы дл€ групп
...
void read_kod() {
		...
		if (flag_run == 0 && valdes[0] >= 1450 && valdes[0] <= 1601) //дл€ реле
		{
		digitalWrite(ngpio,lgpio);//отправл€ю код
		valdes[0] = 0;
		}	
		if (flag_run == 0 && valdes[0] >= 1400 && valdes[0] <= 1441) //дл€ ролет по одной
		{
		flag_run = 1;
	...
	if (flag_run == 2)
	{
		...
		if(tik1_cycle > count_cycle)//если больше
		{
		tik1_cycle = 0;// обнул€ем 
		tik2_cycle ++; ////считаю циклы циклов
		nmbr+=1;// прибавл€ю номер €чейки
		delay(3);//??
...
void startfunc() {
	...
	ets_timer_arm(&esp_timer_rc, 200, 1);// от 200 до 210мс, посто€нна€ работа
		if (sensors_param.cfgdes[1] != 20)
			{
		count1_cycle = sensors_param.cfgdes[1]*5 ;//циклы во врем€ сек
			}
		if (sensors_param.cfgdes[2] != 10)
			{
		half_cycle = sensors_param.cfgdes[2]*5 ;// полуциклы во врем€ сек
			}
}
...
void webfunc(char *pbuf) { //тут обь€сн€лок не будет
